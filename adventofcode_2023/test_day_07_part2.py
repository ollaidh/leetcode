    ...
    def hand_type(self) -> int:
        ...
        for card in self.cards:
            if card not in counter:
                counter[card] = 0
            counter[card] += 1
            if counter[card] > max_counter and card != 'J':
                max_counter = counter[card]
                most_freq = card

        if 'J' in counter and counter['J'] < 5:
            counter[most_freq] += counter['J']
            counter.pop('J', None)

        freqs = [value for _, value in counter.items()]
        max_freq = max(freqs)
...
class TestHands(unittest.TestCase):
    ...
    def test_ln_hands(self):
        ...
        hand1 = Hand('JKKK2', 111)
        hand2 = Hand('QQQQ2', 111)
        self.assertTrue(hand1 < hand2)
    ...
    def test_total_winnings(self):
        ...
        self.assertEqual(5905, total_winnings(hands))
