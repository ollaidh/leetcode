...
def count_arrangements(pack: SpringPack) -> int:
    ...
    if pack.springs.startswith('#'):
        ...
        if pack.springs[damaged_prefix_len] == '.':
            ...
            return count_arrangements(pack)

    #  '#' followed by '?', or starts with '?':
    operational, damaged = replace_element(pack.springs, damaged_prefix_len)
    return count_arrangements(SpringPack(operational, pack.pattern)) + count_arrangements(
        SpringPack(damaged, pack.pattern))
...
def play(input_path: str) -> int:
    ...
    with open(filepath_inp) as f:
        for line in f:
            result += count_arrangements(get_line_parameters(line))
...
if __name__ == '__main__':
    print(play('inputs/test_day_12.dat'))
